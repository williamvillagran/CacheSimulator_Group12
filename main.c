...
int main(int argc, char *argv[]){
    ...
    int physicalMem = atoi (argv[4]);
    int percentageMem = atoi(argv[6]);
    int intructionTimeSlice = atoi(argv[7]);

    char replacementPolicy[30];
    strncpy(replacementPolicy, argv[5], sizeof(replacementPolicy) - 1);
    replacementPolicy[sizeof(replacementPolicy) - 1] = '\0'; //Null-terminate
    

    char traceFile1[30];
    strncpy(traceFile1, argv[8], sizeof(traceFile1) - 1);
